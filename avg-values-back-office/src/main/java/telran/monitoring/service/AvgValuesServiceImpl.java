...
public class AvgValuesServiceImpl implements AvgValuesService{
	...
	static Logger LOG = LoggerFactory.getLogger(AvgValuesService.class);
	...
	public int getAvgPulseValueDates(long patientId, LocalDateTime from, LocalDateTime to) {
		LOG.trace("*avg-values* from time: {}", from);
		LOG.trace("*avg-values* to time: {}", to);
		Criteria criteria = Criteria.where(PATIENT_ID).is(patientId)
				.andOperator(Criteria.where(DATE_TIME).gte(from).lte(to));
		LOG.trace("*avg-values*  all values of patient {} are {}", patientId,
